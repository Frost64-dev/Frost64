...
- When the interrupt flag is clear, external interrupts will be handled after each instruction. When it is set, any external interrupts will be ignored, and will not be saved for when the flag is set again. However, interrupts generated by exceptions will still be handled regardless of the state of the interrupt flag.
- The interrupt flag is automatically set when an external interrupt is being handled, and automatically restored as part of setting the status register on interrupt return.
...
1. IP, STS, and an error code are sent as arguments
